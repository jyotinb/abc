...
class GreenMaster(models.Model):
    ...
    def _calculate_advanced_clamps(self):
        ...
        if self.clamp_type == 'none' and not (self.is_side_coridoors and self.support_hockeys > 0):
            # Check if we still need to calculate for border purlins
            if int(self.bay_side_border_purlin) <= 0 and int(self.span_side_border_purlin) <= 0:
                return
...
💰 GRAND TOTAL: {self.grand_total_cost:.2f}'''
    ...
    def action_view_clamp_details(self):
        ...
        # Border Purlin Clamps Details
        border_details = self._get_border_purlin_details()
        if border_details:
        ...
        # Add border purlin summary
        if self.border_purlin_clamps_summary:
            details.append("")
            details.append("BORDER PURLIN CONFIG:")
            details.append(self.border_purlin_clamps_summary)
    ...
    def _get_border_purlin_details(self):
        """Generate border purlin clamp details for display"""
        details = []
        
        if int(self.bay_side_border_purlin) > 0:
            details.append("BORDER PURLIN CLAMPS - BAY SIDE")
            details.append("-" * 50)
            
            # Front Bay
            if self.width_front_bay_coridoor > 0:
                details.append("Front Bay: Using ASC Pipe Size")
                hockey_count = self._get_front_bay_hockey_count()
                details.append(f"  Hockey Count: {hockey_count}")
            else:
                details.append(f"Front Bay: Thick Column Config = {self.thick_column}")
                half_size, full_size = self._get_bay_side_pipe_sizes('front')
                details.append(f"  Half Clamp Size: {half_size}")
                details.append(f"  Full Clamp Size: {full_size}")
            
            # Back Bay
            if self.width_back_bay_coridoor > 0:
                details.append("Back Bay: Using ASC Pipe Size")
                hockey_count = self._get_back_bay_hockey_count()
                details.append(f"  Hockey Count: {hockey_count}")
            else:
                details.append(f"Back Bay: Thick Column Config = {self.thick_column}")
                half_size, full_size = self._get_bay_side_pipe_sizes('back')
                details.append(f"  Half Clamp Size: {half_size}")
                details.append(f"  Full Clamp Size: {full_size}")
        ...
        if int(self.span_side_border_purlin) > 0:
            details.append("BORDER PURLIN CLAMPS - SPAN SIDE")
            details.append("-" * 50)
            
            # Front Span
            if self.width_front_span_coridoor > 0:
                details.append("Front Span: Using ASC Pipe Size")
                hockey_count = self._get_front_span_hockey_count()
                details.append(f"  Hockey Count: {hockey_count}")
            else:
                details.append(f"Front Span: Thick Column Config = {self.thick_column}")
                half_size, full_size = self._get_span_side_pipe_sizes('front')
                details.append(f"  Half Clamp Size: {half_size}")
                details.append(f"  Full Clamp Size: {full_size}")
            ...
            # Back Span
            if self.width_back_span_coridoor > 0:
                details.append("Back Span: Using ASC Pipe Size")
                hockey_count = self._get_back_span_hockey_count()
                details.append(f"  Hockey Count: {hockey_count}")
            else:
                details.append(f"Back Span: Thick Column Config = {self.thick_column}")
                half_size, full_size = self._get_span_side_pipe_sizes('back')
                details.append(f"  Half Clamp Size: {half_size}")
                details.append(f"  Full Clamp Size: {full_size}")
            ...
            if self.no_anchor_frame_lines >= 1:
                details.append("")
                details.append(f"ANCHOR FRAME LINES: {self.no_anchor_frame_lines}")
                if self.no_anchor_frame_lines == 1:
                    details.append("  Note: ASC and Anchor Frame at same line")
                big_column_size = self._get_big_column_pipe_size()
                details.append(f"  Big Column Size: {big_column_size}")
