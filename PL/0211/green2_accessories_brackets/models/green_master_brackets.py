...
class GreenMasterBrackets(models.Model):
    ...
    def _calculate_gi_patti(self):
        """
        Calculate GI Patti components
        
        Logic:
        - Only if last_span_gutter is False (not last span gutter)
        - Nos = (No of Bays + 1) * 2
        """
        if self.last_span_gutter:
            _logger.info("GI Patti not applicable (last span gutter is enabled)")
            return
        
        if self.no_of_bays <= 0:
            _logger.warning("Insufficient data for GI Patti calculation")
            return
        
        # Calculate quantity
        qty = (self.no_of_bays + 1) * 2
        ...
        _logger.info(f"Created {qty} GI Patti (No Last Span Gutter)")
