...
class GreenMasterNutbolts(models.Model):
    ...
    def _calculate_bracket_nutbolts(self):
        """Calculate nut-bolts for all bracket types - unified logic"""
        # Check if any bracket type exists
        has_brackets = (
            self.gutter_bracket_type in ['arch', 'f_bracket'] or
            self.brackets_component_ids.filtered(lambda c: 'Bracket' in c.name or 'GI Patti' in c.name)
        )
        
        if not has_brackets or self.gutter_bracket_type == 'none':
            _logger.info("No brackets configured or bracket type is 'none' - skipping bracket nut bolts")
            return
        
        _logger.info(f"=== Calculating Bracket Nut Bolts (Type: {self.gutter_bracket_type}) ===")
        
        # A. Calculate Arch/Bottom Bolts based on components
        self._calculate_arch_bottom_bolts()
        
        # B. Calculate Column Bolts based on column components
        self._calculate_column_bolts_for_brackets()

    def _calculate_arch_bottom_bolts(self):
        """
        Calculate bolts for Big Arch, Small Arch, and Bottom Chord components
        
        Logic:
        - If arch_pressed OR bottom_pressed = True: Size = 10x40
        - Else: Size = 10 × (component_pipe_size + 15mm) rounded
        - Quantity = 2 × component.nos (except Bottom IL Singular = 4 × nos)
        """
        _logger.info("--- Calculating Arch/Bottom Bolts ---")
        
        # 1. Big Arch Bolts
        big_arch = self.truss_component_ids.filtered(lambda c: c.name == 'Big Arch')
        if big_arch and big_arch.nos > 0:
            if self.arch_pressed or self.bottom_pressed:
                size = '10x40'
                _logger.info(f"Big Arch: Using pressed size 10x40")
            else:
                pipe_size = self._get_pipe_size_from_component('Big Arch', 'truss')
                calculated_length = ceil(pipe_size + 15) if pipe_size > 0 else 40
                size = self._round_to_available_bolt_size(calculated_length)
                _logger.info(f"Big Arch: Calculated size {size} (pipe: {pipe_size}mm)")
            
            quantity = big_arch.nos * 2
            self._create_nutbolt_component(f'Nut Bolt {size} - Big Arch', size, quantity)
            _logger.info(f"✅ Big Arch: {size} × {quantity} units")
        
        # 2. Small Arch Bolts
        small_arch = self.truss_component_ids.filtered(lambda c: c.name == 'Small Arch')
        if small_arch and small_arch.nos > 0:
            if self.arch_pressed or self.bottom_pressed:
                size = '10x40'
                _logger.info(f"Small Arch: Using pressed size 10x40")
            else:
                pipe_size = self._get_pipe_size_from_component('Small Arch', 'truss')
                calculated_length = ceil(pipe_size + 15) if pipe_size > 0 else 40
                size = self._round_to_available_bolt_size(calculated_length)
                _logger.info(f"Small Arch: Calculated size {size} (pipe: {pipe_size}mm)")
            
            quantity = small_arch.nos * 2
            self._create_nutbolt_component(f'Nut Bolt {size} - Small Arch', size, quantity)
            _logger.info(f"✅ Small Arch: {size} × {quantity} units")
        
        # 3. Bottom Chord Bolts - All types
        bottom_chord_types = [
            ('Bottom Chord Anchor Frame Singular', 'Bottom AF Singular'),
            ('Bottom Chord Anchor Frame Male', 'Bottom AF Male'),
            ('Bottom Chord Anchor Frame Female', 'Bottom AF Female'),
            ('Bottom Chord Inner Line Singular', 'Bottom IL Singular'),
            ('Bottom Chord Inner Line Male', 'Bottom IL Male'),
            ('Bottom Chord Inner Line Female', 'Bottom IL Female'),
        ]
        ...
        for component_name, display_name in bottom_chord_types:
            ...
            if component and component.nos > 0:
                if self.arch_pressed or self.bottom_pressed:
                    size = '10x40'
                    _logger.info(f"{display_name}: Using pressed size 10x40")
                else:
                    pipe_size = self._get_pipe_size_from_component(component_name, 'truss')
                    calculated_length = ceil(pipe_size + 15) if pipe_size > 0 else 40
                    size = self._round_to_available_bolt_size(calculated_length)
                    _logger.info(f"{display_name}: Calculated size {size} (pipe: {pipe_size}mm)")
                
                # Special case: Inner Line Singular uses ×4
                ...
                self._create_nutbolt_component(f'Nut Bolt {size} - {display_name}', size, quantity)
                _logger.info(f"✅ {display_name}: {size} × {quantity} units (nos: {component.nos} × {multiplier})")

    def _calculate_column_bolts_for_brackets(self):
        """
        Calculate column bolts based on actual column components (NOT brackets)
        
        Logic:
        - Size = 10 × (Column Pipe Size + 15mm) rounded
        - Quantity = 2 × column_component.nos
        """
        _logger.info("--- Calculating Column Bolts ---")
        
        column_types = [
            ('Thick Columns', 'Thick Column'),
            ('Main Columns', 'Main Column'),
            ('AF Main Columns', 'AF Main Column'),
        ]
        ...
        for component_name, display_name in column_types:
            ...
            if component and component.nos > 0:
                ...
                if pipe_size > 0:
                    ...
                    quantity = component.nos * 2
                    self._create_nutbolt_component(f'Nut Bolt {size} - {display_name}', size, quantity)
                    _logger.info(f"✅ {display_name}: {size} × {quantity} units (nos: {component.nos} × 2)")
                else:
                    _logger.warning(f"⚠️ {display_name}: No pipe size found")
    ...
    def _calculate_gi_patti_nutbolts(self):
        ...
        if not hasattr(self, 'brackets_component_ids'):
            return
        
        gi_patti = self.brackets_component_ids.filtered(lambda c: 'GI Patti' in c.name)
