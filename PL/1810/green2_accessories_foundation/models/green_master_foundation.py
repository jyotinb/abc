...
class GreenMasterFoundation(models.Model):
    ...
    def _calculate_foundation_rods(self):
        ...
        if not self.enable_foundation_rods or self.foundation_length <= 0:
            return
    ...
    @api.onchange('foundation_length')
    def _onchange_foundation_length(self):
        """Disable foundation rods when foundation_length is 0"""
        if self.foundation_length <= 0:
            self.enable_foundation_rods = False
