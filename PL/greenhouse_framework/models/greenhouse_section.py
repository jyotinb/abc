...
class GreenhouseSectionTemplate(models.Model):
    ...
    @api.model
    def get_all_calculation_codes(self):
        """Get all calculation codes across all sections for validation"""
        all_codes = set()
        for section in self.search([]):
            for rule in section.get_calculation_rules():
                all_codes.add(rule.get('code'))
        return all_codes
    ...
    def create_sample_section(self):
            ...
            'input_fields': json.dumps([
                {
                    ...
                    "max": 100,
                    "sequence": 10,
                    "help": "This is a sample input field"
                }
            ]),
            'calculation_rules': json.dumps([
                {
                    "code": "SAMPLE_CALC",
                    "name": "Sample Calculation",
                    "formula": "GET('sample_input') * 2",
                    "sequence": 10,
                    "length_per_unit": 1.0
                }
            ])
        ...
        existing = self.search([('code', '=', 'sample')], limit=1)
        if not existing:
            ...
            return {
                ...
                'params': {
                    'title': 'Info',
                    'message': 'Sample section already exists',
                    'type': 'info',
                }
            }