...
class GreenMaster(models.Model):
    ...
    def _calculate_advanced_clamps(self):
        ...
        if self.clamp_type == 'none' and not (self.is_side_coridoors and self.support_hockeys > 0):
            ...
            if (int(self.bay_side_border_purlin) <= 0 and 
                ...
                self.arch_middle_purlin_small_arch == '0' and
                not (self.is_bottom_chord and int(self.v_support_bottom_chord_frame) > 0)):
                return
    ...
    def _accumulate_v_support_main_column_clamps(self, accumulator):
        """Accumulate V Support to Main Column and AF Main Column clamps with thick column consideration"""
        
        # Check if V Support is configured
        ...
        # Skip if no columns configured
        if no_of_bays <= 0 or no_of_spans <= 0:
            return
        
        # FULL CLAMPS CALCULATION (End Connections)
        
        if thick_config == '0':  # None - No thick columns
        ...
        elif thick_config == '4':  # 4 Side
            # All columns are thick
            if thick_column_size:
                self._add_to_clamp_accumulator(accumulator, 'Full Clamp', thick_column_size, (no_of_bays + 1) * 2)
        
        # HALF CLAMPS CALCULATION (Internal Connections - only if spans > 1)
        
        if no_of_spans > 1:
            if thick_config == '0':  # None
                # Standard calculation
                if no_of_af_lines > 0 and af_main_column_size:
                ...
                # Internal connections not affected by corner thick columns
                if no_of_af_lines > 0 and af_main_column_size:
                    af_half_qty = no_of_af_lines * (no_of_spans - 1)
                    self._add_to_clamp_accumulator(accumulator, 'Half Clamp', af_main_column_size, af_half_qty)
                ...
                # All internal connections use thick column size
                if thick_column_size:
                ...
                # Span side configuration doesn't affect internal bay connections
                if no_of_af_lines > 0 and af_main_column_size:
                    af_half_qty = no_of_af_lines * (no_of_spans - 1)
                    self._add_to_clamp_accumulator(accumulator, 'Half Clamp', af_main_column_size, af_half_qty)
    ...
    def get_clamp_calculation_details(self):
        ...
        # NEW: Calculate V Support to Main Column Clamps
        # This should be calculated regardless of clamp type if V support is configured
        if self.is_bottom_chord and int(self.v_support_bottom_chord_frame) > 0:
        ...
        # Calculate Arch Middle Purlin Clamps
    ...
    def _get_w_type_purlin_details(self, start_sequence):
        """Get W Type purlin clamp details"""
        details = []
    ...
    def _get_common_arch_details(self, category, start_sequence):
        """Get common arch clamp details for both W and M types"""
        details = []
        sequence = start_sequence
    ...
    def _get_v_support_main_column_details(self, sequence):
        """Get V Support to Main Column clamp details with thick column support"""
        details = []
        ...
        # FULL CLAMPS - End Connections
        
        if thick_config == '0':  # None
            # Standard calculation without thick columns
            if no_of_af_lines > 0 and af_main_column_size:
                af_full_qty = no_of_af_lines * 2
                details.append({
                    'sequence': sequence,
                    'category': 'V SUPPORT TO MAIN COLUMN - FULL CLAMPS',
                    'component': 'AF Main Column (End Connections)',
                    'clamp_type': 'Full Clamp',
                    'size': af_main_column_size,
                    'quantity': int(af_full_qty),
                    'formula': f"{no_of_af_lines} AF Lines × 2",
            ...
            if main_column_size:
                ...
                if main_full_qty > 0:
                    details.append({
                        'sequence': sequence,
                        'category': 'V SUPPORT TO MAIN COLUMN - FULL CLAMPS',
                        'component': 'Main Column (End Connections)',
                        'clamp_type': 'Full Clamp',
                        'size': main_column_size,
                        'quantity': int(main_full_qty),
                        'formula': f"({no_of_bays + 1} - {no_of_af_lines}) × 2",
                        'unit_price': self._get_clamp_price('Full Clamp', main_column_size),
                    })
                    sequence += 10
        
        elif thick_config == '1':  # 4 Corners
            # Show thick column clamps
            if thick_column_size:
                details.append({
                    'sequence': sequence,
                    'category': 'V SUPPORT TO MAIN COLUMN - FULL CLAMPS',
                    'component': 'Thick Column (4 Corners)',
                    'clamp_type': 'Full Clamp',
                    'size': thick_column_size,
                    'quantity': 4,
                    'formula': "4 Corner Positions",
                    'unit_price': self._get_clamp_price('Full Clamp', thick_column_size),
                })
                sequence += 10
            
            # Calculate remaining AF and Main columns
            corners_from_af = min(4, no_of_af_lines * 2)
            corners_from_main = 4 - corners_from_af
            ...
            if no_of_af_lines > 0 and af_main_column_size:
                ...
                if af_full_qty > 0:
                    details.append({
                        'sequence': sequence,
                        'category': 'V SUPPORT TO MAIN COLUMN - FULL CLAMPS',
                        'component': 'AF Main Column (After Corners)',
                        'clamp_type': 'Full Clamp',
                        'size': af_main_column_size,
                        'quantity': int(af_full_qty),
                        'formula': f"({no_of_af_lines} × 2) - {corners_from_af} corners",
                        'unit_price': self._get_clamp_price('Full Clamp', af_main_column_size),
                    })
                    sequence += 10
            ...
            if main_column_size:
                ...
                if main_full_qty > 0:
                    details.append({
                        'sequence': sequence,
                        'category': 'V SUPPORT TO MAIN COLUMN - FULL CLAMPS',
                        'component': 'Main Column (After Corners)',
                        'clamp_type': 'Full Clamp',
                        'size': main_column_size,
                        'quantity': int(main_full_qty),
                        'formula': f"(({no_of_bays + 1} - {no_of_af_lines}) × 2) - {corners_from_main} corners",
                        'unit_price': self._get_clamp_price('Full Clamp', main_column_size),
                    })
                    sequence += 10
            ...
            if thick_column_size:
                details.append({
                    'sequence': sequence,
                    'category': 'V SUPPORT TO MAIN COLUMN - FULL CLAMPS',
                    'component': 'Thick Column (All Bay Side)',
                    'clamp_type': 'Full Clamp',
                    'size': thick_column_size,
                    'quantity': int((no_of_bays + 1) * 2),
                    'formula': f"({no_of_bays + 1}) × 2 (All Bay Side)",
                    'unit_price': self._get_clamp_price('Full Clamp', thick_column_size),
                })
                sequence += 10
        
        elif thick_config == '3':  # 2 Span Side
            # Standard for bay side (span config doesn't affect bay)
            if no_of_af_lines > 0 and af_main_column_size:
                af_full_qty = no_of_af_lines * 2
                details.append({
                    'sequence': sequence,
                    'category': 'V SUPPORT TO MAIN COLUMN - FULL CLAMPS',
                    'component': 'AF Main Column (Bay Side)',
                    'clamp_type': 'Full Clamp',
                    'size': af_main_column_size,
                    'quantity': int(af_full_qty),
                    'formula': f"{no_of_af_lines} AF Lines × 2",
            ...
            if main_column_size:
                ...
                if main_full_qty > 0:
                    details.append({
                        'sequence': sequence,
                        'category': 'V SUPPORT TO MAIN COLUMN - FULL CLAMPS',
                        'component': 'Main Column (Bay Side)',
                        'clamp_type': 'Full Clamp',
                        'size': main_column_size,
                        'quantity': int(main_full_qty),
                        'formula': f"({no_of_bays + 1} - {no_of_af_lines}) × 2",
            ...
            if thick_column_size:
                details.append({
                    'sequence': sequence,
                    'category': 'V SUPPORT TO MAIN COLUMN - FULL CLAMPS',
                    'component': 'Thick Column (All Sides)',
                    'clamp_type': 'Full Clamp',
                    'size': thick_column_size,
                    'quantity': int((no_of_bays + 1) * 2),
                    'formula': f"({no_of_bays + 1}) × 2 (All Columns)",
                    'unit_price': self._get_clamp_price('Full Clamp', thick_column_size),
                })
                sequence += 10
        
        # HALF CLAMPS - Internal Connections (similar updates needed)
        if no_of_spans > 1:
            # Add similar thick column handling for half clamps...
            # (Implementation follows same pattern as full clamps)
            pass
        
        # Add configuration summary
        thick_config_name = dict(self._fields['thick_column'].selection)[thick_config] if thick_config else 'None'
        details.append({
            'sequence': sequence + 100,
            'category': 'V SUPPORT TO MAIN COLUMN - CONFIGURATION',
            'component': 'Settings',
            'clamp_type': 'Info',
            'size': 'N/A',
            'quantity': 0,
            'formula': f'Thick Column: {thick_config_name} | Total Frames: {no_of_bays + 1} | AF Lines: {no_of_af_lines} | Spans: {no_of_spans}',
            'unit_price': 0,
        })
    ...
    def _get_cross_bracing_details(self, sequence):
        """Get Cross Bracing clamp details"""
        details = []
    ...
    def _get_asc_bracket_details(self, sequence):
        """Get ASC Bracket (Gutter Arch Bracket) clamp details"""
        details = []
        
        # Check if Bay Side clamps required
        has_bay_asc = self.width_front_bay_coridoor > 0 or self.width_back_bay_coridoor > 0
    ...
    def _get_border_purlin_details(self, sequence):
        """Get Border Purlin clamp details"""
        details = []
    ...
    def _get_bay_side_border_details(self, sequence):
        """Get Bay Side Border Purlin clamp details"""
        details = []
        bay_side_value = int(self.bay_side_border_purlin)
        ...
        if self.width_front_bay_coridoor > 0:
            ...
            if half_size and full_size:
                ...
                details.append({
                    'sequence': sequence,
                    'category': 'BORDER PURLIN - BAY SIDE',
                    'component': f'Front Bay (Thick Config: {self.thick_column})',
                    'clamp_type': 'Half Clamp',
                    'size': half_size,
                    'quantity': int(half_qty),
        ...
        # ... (repeat similar pattern for back bay)
        
        return details

    def _get_span_side_border_details(self, sequence):
        """Get Span Side Border Purlin clamp details"""
        details = []
        span_side_value = int(self.span_side_border_purlin)
        big_frame_multiplier = int(self.no_column_big_frame) + 1
        
        # Similar implementation as bay side but for span side
        # ... (implement following the same pattern)
        
        return details

    def _get_anchor_frame_details(self, sequence):
        """Get Anchor Frame Line clamp details"""
        details = []
        span_side_value = int(self.span_side_border_purlin)
    ...
    def _get_arch_middle_purlin_details(self, sequence):
        """Get Arch Middle Purlin clamp details"""
        details = []
    ...
    def _get_big_arch_middle_purlin_details(self, sequence):
        """Get Big Arch Middle Purlin clamp details"""
        details = []
    ...
    def _get_small_arch_middle_purlin_details(self, sequence):
        """Get Small Arch Middle Purlin clamp details"""
        details = []
    ...
    def _get_af_main_column_pipe_size(self):
        """Get AF Main Column pipe size"""
        af_main_columns = self.frame_component_ids.filtered(
        ...
        # Fallback to regular main column size if AF Main Columns not found
        return self._get_main_column_pipe_size()
    
    def _get_clamp_price(self, clamp_type, size):
        """Get unit price for a clamp"""
        master_record = self.env['accessories.master'].search([
            ('name', '=', clamp_type),
            ('category', '=', 'clamps'),
    ...
    def _get_bay_side_pipe_sizes(self, location='front'):
        ...
        try:
            ...
            # No thick columns or Span Side configurations
            else:  # thick_config in ['0', '3']
                half_size = self._get_main_column_pipe_size()
    ...
    def _get_span_side_pipe_sizes(self, location='front'):
        ...
        try:
            ...
            # No thick columns or Bay Side configurations
            else:  # thick_config in ['0', '2']
                half_size = self._get_main_column_pipe_size()
                full_size = self._get_main_column_pipe_size()
...
💰 GRAND TOTAL: {self.grand_total_cost:.2f}'''
    ...
    def action_view_clamp_details(self):
        """Show detailed clamp calculation breakdown in wizard view"""
