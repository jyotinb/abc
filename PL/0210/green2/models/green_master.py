...
{chr(10).join(components_without_pipes) if components_without_pipes else 'None'}"""
    ...
    def _calculate_all_components(self):
        ...
        # Only calculate if curtains are configured OR if guard/guard box is enabled
        ...
        # Update side_screen_guard calculation:
        ...
        if self.foundation_length > 0:
            ...
            # ADD THIS: Quadruple Columns Foundations
            if no_quadraple_columns > 0:
                component_vals.append(self._create_component_val(
    ...
    def _check_span_bay_asc_dimensions(self):
        ...
        for record in self:
            ...
            if (record.total_span_length <= 0 or record.total_bay_length <= 0 or 
                ...
                # Skip this validation if basic dimensions are not set properly
                # The _check_positive_dimensions constraint will handle it
                return
            ...
            if record.is_side_coridoors:
                ...
                if record.bay_width and record.bay_width > 0:
                    # Calculate effective span length after ASC corridors
                    ...
                    if effective_span_length <= 0:
                        ...
                        # Check if result is not an integer (allowing small floating point errors)
                ...
                if record.span_width and record.span_width > 0:
                    # Calculate effective bay length after ASC corridors
            ...
            # Also validate when ASC is disabled but basic dimensions should still be integers
            else:
                errors = []
                
                # Check bay calculation without ASC
                if record.bay_width and record.bay_width > 0 and record.total_span_length > 0:
                    bay_result = record.total_span_length / record.bay_width
                    if abs(bay_result - round(bay_result)) > 0.001:
                        errors.append(
                            f"Bay calculation error: {record.total_span_length} / "
                            f"{record.bay_width} = {bay_result:.2f} (must be integer)"
                ...
                # Check span calculation without ASC
                if record.span_width and record.span_width > 0 and record.total_bay_length > 0:
                    span_result = record.total_bay_length / record.span_width
                    if abs(span_result - round(span_result)) > 0.001:
                        errors.append(
                            f"Span calculation error: {record.total_bay_length} / "
                            f"{record.span_width} = {span_result:.2f} (must be integer)"
    ...
    def _onchange_side_screen_guard(self):
        ...
        if not self.side_screen_guard:
            # If guard is disabled, guard box should also be disabled
            self.side_screen_guard_box = False
            self.no_side_screen_guard_box = 0         
...
# Add these updated methods to your green_master.py file to handle door components properly

    def action_clear_all_components(self):
        """Simple clear all components including doors without recalculation"""
        try:
            # Include door components in the count
            component_count = len(self.asc_component_ids + self.frame_component_ids + 
                                 self.truss_component_ids + self.side_screen_component_ids + 
                                 self.lower_component_ids)
            
            # Check if door module is installed and add door components
            if hasattr(self, 'door_component_ids'):
                component_count += len(self.door_component_ids)
            
            # Clear all components including doors
            clear_vals = {
            ...
            # Add door components if the field exists
            if hasattr(self, 'door_component_ids'):
                clear_vals['door_component_ids'] = [(5, 0, 0)]
    ...
    def action_reset_from_component_page(self):
        """Reset and recalculate from component page including doors"""
        try:
            # Step 1: Clear all components including doors
            clear_vals = {
                'asc_component_ids': [(5, 0, 0)],
                'frame_component_ids': [(5, 0, 0)],
                'truss_component_ids': [(5, 0, 0)],
            ...
            # Step 2: Recalculate (will call door module's extended method automatically)
            ...
            return {
                ...
                'params': {
                    ...
                    'message': 'All components have been reset and recalculated.',
                    'type': 'success',
                }
            }
        except Exception as e:
    ...
    def action_clear_pipe_selections(self):
        """Clear all pipe selections but keep components including doors"""
        try:
            count = 0
            all_components = (self.asc_component_ids + self.frame_component_ids + 
                             self.truss_component_ids + self.side_screen_component_ids + 
    ...
    def action_recalculate_smart(self):
        """Smart recalculation that preserves pipe selections including doors"""
        try:
            # Save current pipe selections including doors
            saved_pipes = {}
            all_components = (self.asc_component_ids + self.frame_component_ids + 
                             self.truss_component_ids + self.side_screen_component_ids + 
                             self.lower_component_ids)
            ...
            for comp in all_components:
                key = f"{comp.section}_{comp.name}"
                if comp.pipe_id:
                    saved_pipes[key] = comp.pipe_id.id
            ...
            # This will call the door module's extended version automatically
            self._calculate_all_components()
            
            # Restore pipe selections
            restored = 0
            new_components = (self.asc_component_ids + self.frame_component_ids + 
                             self.truss_component_ids + self.side_screen_component_ids + 
                             self.lower_component_ids)
            
            # Add door components if they exist
            if hasattr(self, 'door_component_ids'):
                new_components = new_components + self.door_component_ids
            
            for comp in new_components:
                key = f"{comp.section}_{comp.name}"
                if key in saved_pipes:
                    comp.pipe_id = saved_pipes[key]
                    restored += 1
            ...
            return {
                ...
                'params': {
                    ...
                    'message': f'Components recalculated. Restored {restored} pipe selections.',
                    'type': 'success',
                }
            }
        except Exception as e:
    ...
    def action_calculate_process(self):
        """Enhanced recalculate process that handles door components"""
        for record in self:
            try:
                # Include door components in saved selections
                ...
                # Add door count if module is installed
                if hasattr(record, 'door_component_ids'):
                    component_counts_before['doors'] = len(record.door_component_ids or [])
                
                record._robust_clear_components()
                record._calculate_all_components()  # This calls door module's extended version
                ...
                # Add door count if module is installed
                if hasattr(record, 'door_component_ids'):
                    component_counts_after['doors'] = len(record.door_component_ids or [])
    ...
    def _robust_clear_components(self):
        """Enhanced clear method that handles door components"""
        ...
        for section_field, section_name in [
            ('asc_component_ids', 'asc'),
            ('frame_component_ids', 'frame'),
            ('truss_component_ids', 'truss'),
            ('side_screen_component_ids', 'side_screen'),
            ('lower_component_ids', 'lower'),
        ]:
            try:
                components = getattr(self, section_field, False)
                if components:
                    components.unlink()
                    sections_cleared.append(section_name)
            except Exception as e:
                sections_failed.append(f'{section_name}: {str(e)}')
        
        # Clear door components if they exist
        if hasattr(self, 'door_component_ids'):
            try:
                if self.door_component_ids:
                    self.door_component_ids.unlink()
                    sections_cleared.append('doors')
            except Exception as e:
                sections_failed.append(f'doors: {str(e)}')
