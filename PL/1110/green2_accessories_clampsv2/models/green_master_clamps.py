...
class GreenMasterClamps(models.Model):
    ...
    def _accumulate_purlin_clamps(self, accumulator):
        """
        FIXED METHOD: Accumulate purlin clamps for both W and M types
        Uses no_of_bays instead of component nos field
        Keeps Big and Small separate even with same size
        """
        # Get Arch data (for CLAMP SIZE only)
        big_arch_data = self._get_big_arch_data()
        small_arch_data = self._get_small_arch_data()
        
        # Calculate total clamp positions based on structure
        # Each bay needs 2 clamp positions (one per side)
        total_clamp_positions = self.no_of_bays * 2
        
        # Big Purlin clamps
        if big_arch_data['size'] and self.big_purlin_clamp_type_first:
            # First Type: End positions
            qty_first = self.no_of_spans * 2
            clamp_name = 'Full Clamp' if self.big_purlin_clamp_type_first == 'full_clamp' else 'L Joint'
            
            # Add source identifier to prevent combining with Small Arch
            component_name = f"{clamp_name} (Big Arch Purlin)"
            self._add_to_clamp_accumulator_separate(
                accumulator, component_name, big_arch_data['size'], qty_first
            )
            
            # Second Type: Intermediate positions
            if self.big_purlin_clamp_type_second:
                qty_second = total_clamp_positions - qty_first
                if qty_second > 0:
                    clamp_name = 'Half Clamp' if self.big_purlin_clamp_type_second == 'half_clamp' else 'T Joint'
                    component_name = f"{clamp_name} (Big Arch Purlin)"
                    self._add_to_clamp_accumulator_separate(
                        accumulator, component_name, big_arch_data['size'], qty_second
                    )
        
        # Small Purlin clamps (same logic)
        if small_arch_data['size'] and self.small_purlin_clamp_type_first:
            ...
            # Add source identifier to prevent combining with Big Arch
            component_name = f"{clamp_name} (Small Arch Purlin)"
            self._add_to_clamp_accumulator_separate(
                accumulator, component_name, small_arch_data['size'], qty_first
            )
            ...
            if self.small_purlin_clamp_type_second:
                ...
                if qty_second > 0:
                    ...
                    component_name = f"{clamp_name} (Small Arch Purlin)"
                    self._add_to_clamp_accumulator_separate(
                        accumulator, component_name, small_arch_data['size'], qty_second
                    )
    ...
    def _add_to_clamp_accumulator(self, accumulator, clamp_type, size, quantity):
        """Add clamps to accumulator (combines if same type and size)"""
        if quantity <= 0:
            return
    ...
    def _add_to_clamp_accumulator_separate(self, accumulator, component_name, size, quantity):
        """Add clamps to accumulator WITHOUT combining (keeps separate by name)"""
        if quantity <= 0:
            return
        
        # Use component_name as part of key to keep Big and Small separate
    ...
    def _create_clamp_components_from_accumulator(self, accumulator):
        ...
        for (clamp_identifier, size), quantity in accumulator.items():
            ...
            # e.g., "Full Clamp (Big Arch Purlin)" -> "Full Clamp"
            if '(' in clamp_identifier:
                base_clamp_type = clamp_identifier.split('(')[0].strip()
                component_name = clamp_identifier  # Use full name with source
            else:
                base_clamp_type = clamp_identifier
                component_name = f"{clamp_identifier} - {size}"
            
            # Find matching master record using base type
