...
class ClampCalculationDetail(models.TransientModel):
    ...
    def _generate_type_summary(self):
        ...
        for line in self.calculation_line_ids:
            if line.clamp_type != 'Info' and line.quantity > 0:
                clamp_type = line.clamp_type
                if clamp_type not in type_data:
                    type_data[clamp_type] = {
                        'count': 0,
                        'sizes': {},
                        'total_cost': 0.0
                    }
                
                type_data[clamp_type]['count'] += line.quantity
                type_data[clamp_type]['total_cost'] += line.total_cost
                
                # Track sizes for this type
                if line.size not in type_data[clamp_type]['sizes']:
                    type_data[clamp_type]['sizes'][line.size] = 0
                type_data[clamp_type]['sizes'][line.size] += line.quantity
        
        # Generate HTML table
        html = '<table class="table table-sm table-bordered">'
        html += '<thead><tr>'
        html += '<th>Clamp Type</th><th>Total Quantity</th><th>Sizes Used</th><th>Total Cost</th>'
        html += '</tr></thead><tbody>'
        
        for clamp_type, data in sorted(type_data.items()):
            sizes_str = ', '.join([f"{size}: {qty}" for size, qty in sorted(data['sizes'].items())])
            html += f'<tr>'
            html += f'<td><strong>{clamp_type}</strong></td>'
            html += f'<td>{data["count"]}</td>'
            html += f'<td>{sizes_str}</td>'
            html += f'<td><strong>{data["total_cost"]:.2f}</strong></td>'
            html += f'</tr>'
        
        # Add totals row
        total_quantity = sum(data['count'] for data in type_data.values())
        total_cost = sum(data['total_cost'] for data in type_data.values())
        html += f'<tr class="table-primary">'
        html += f'<td><strong>TOTAL</strong></td>'
        html += f'<td><strong>{total_quantity}</strong></td>'
        html += f'<td>-</td>'
        html += f'<td><strong>{total_cost:.2f}</strong></td>'
        html += f'</tr>'
