...
class ProductAdditionalCostWizard(models.TransientModel):
    ...
    level1Add = fields.Float('A+ Addition', digits='Product Price')
    level2Add = fields.Float('A Addition', digits='Product Price')
    level3Add = fields.Float('B Addition', digits='Product Price')
    level4Add = fields.Float('C Addition', digits='Product Price')
    level1price = fields.Float('A+ Price', digits='Product Price', compute='_compute_level_prices', readonly=True)
    level2price = fields.Float('A Price', digits='Product Price', compute='_compute_level_prices', readonly=True)
    level3price = fields.Float('B Price', digits='Product Price', compute='_compute_level_prices', readonly=True)
    level4price = fields.Float('C Price', digits='Product Price', compute='_compute_level_prices', readonly=True)
    ...
    calculate_operation_cost = fields.Boolean("Calculate Operation Cost", default=False)
    total_opertation_cost_manual = fields.Float("Manual Operation Cost(Not calculated)")
        ...
        if self._context.get('active_id'):
            ...
            if line.state != 'draft':
                ...
                res.update({
                    ...
                    'calculate_operation_cost': line.calculate_operation_cost or (line.product_id.calculate_operation_cost or False),
                    'total_opertation_cost_manual': line.total_opertation_cost_manual or (line.total_opertation_cost_manual or 0.0),
                })
            ...
            if hasattr(line.product_id, 'calculate_operation_cost'):
                res['calculate_operation_cost'] = line.product_id.calculate_operation_cost if hasattr(line.product_id, 'calculate_operation_cost') else False
            else:
                res['calculate_operation_cost'] = False
            
            if hasattr(line.product_id, 'total_opertation_cost_manual'):
                res['total_opertation_cost_manual'] = line.product_id.total_opertation_cost_manual if hasattr(line.product_id, 'total_opertation_cost_manual') else 0.0
            else:
                res['total_opertation_cost_manual'] = 0.0
    ...
    def action_save_additional_costs(self):
        ...
        values = {
            ...
            'calculate_operation_cost': self.calculate_operation_cost,
            'total_opertation_cost_manual': self.total_opertation_cost_manual,
        }
