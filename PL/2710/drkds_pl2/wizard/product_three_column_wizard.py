...
class ProductThreeColumnWizard(models.TransientModel):
    ...
    price_level = fields.Selection([
        ('1', 'A+ Price'),  # Updated label
        ('2', 'A Price'),   # Updated label
        ('3', 'B Price'),   # Updated label
        ('4', 'C Price'),   # Updated label
    ...
    def action_select_all_products(self):
        ...
        self.ensure_one()
        self.selected_product_ids = [(6, 0, self.available_product_ids.ids)]
        
        # If using custom order, update sequence records
        if self.product_order == 'custom':
            self._onchange_product_order()
    ...
    def action_deselect_all_products(self):
        """Deselect all products"""
        self.ensure_one()
        self.selected_product_ids = [(5, 0, 0)]
        
        # Clear sequence records
        self.product_sequence_ids = [(5, 0, 0)]
